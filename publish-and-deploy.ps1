...
aws cloudformation package --template-file cloudformation.template.yaml --output-template-file $tempFolder/packaged-template.yaml --s3-bucket $buckets[$EnvType] --profile $NamedProfile
...
aws cloudformation deploy --template-file $tempFolder/packaged-template.yaml --stack-name $StackName --capabilities CAPABILITY_IAM --s3-bucket $buckets[$EnvType] --parameter-overrides EnvType=$EnvType EnvName=$EnvName --profile $NamedProfile
